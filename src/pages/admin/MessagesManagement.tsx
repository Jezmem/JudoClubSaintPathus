...
const MessagesManagement: React.FC = () => {
  const { data: messagesResponse, loading, error, refetch } = useApi(() => contactAPI.getAll({ limit: 100 }));
  const { mutate: updateStatus } = useApiMutation((params: { id: number; status: string }) => contactAPI.updateStatus(params.id, params.status));
  const { mutate: deleteMessage } = useApiMutation(contactAPI.delete);
  ...
  const openDetail = (message: any) => {
    setSelectedMessage(message);
    setIsDetailOpen(true);
    
    // Mark as read if unread
    if (message.status === 'unread') {
      updateStatus({ id: message.id, status: 'read' }).then(() => {
        refetch();
      });
    }
  };
  ...
  const markAsReplied = (id: number) => {
    updateStatus({ id, status: 'replied' }).then(() => {
      refetch();
    });
  };
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-center justify-between pt-6 border-t border-gray-200">
                <div className="flex space-x-3">
                  <a
                    href={`mailto:${selectedMessage.email}?subject=Re: ${selectedMessage.subject}&body=Bonjour ${selectedMessage.name},%0D%0A%0D%0AMerci pour votre message.%0D%0A%0D%0ACordialement,%0D%0AJudo Club Saint Pathus`}
