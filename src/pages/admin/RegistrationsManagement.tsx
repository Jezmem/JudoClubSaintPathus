...
const RegistrationsManagement: React.FC = () => {
  const { data: registrationsResponse, loading, error, refetch } = useApi(() => registrationAPI.getAll({ limit: 100 }));
  const { mutate: updateRegistration } = useApiMutation((params: { id: number; data: any }) => registrationAPI.update(params.id, params.data));
  ...
  const filteredRegistrations = registrations.filter((registration: any) => {
    const matchesSearch = registration.user?.firstName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const openDetail = (registration: any) => {
  ...
  const updateStatus = (id: number, status: 'approved' | 'rejected') => {
    updateRegistration({ id, data: { status } }).then(() => {
      refetch();
    });
  };
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [
      ...
      ...filteredRegistrations.map((reg: any) => [
        ...
        reg.user?.email,
        reg.user?.phone,
        calculateAge(reg.user?.dateOfBirth || ''),
                  ...
                  >
                    <td className="px-6 py-4">
                      <div>
                        ...
                        <div className="flex items-center space-x-1 mt-1">
                          ...
                          <span className="text-xs text-gray-500">{registration.user?.phone}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div>
                        <span className="font-medium text-gray-900">{calculateAge(registration.user?.dateOfBirth || '')} ans</span>
                        {isMinor(registration.user?.dateOfBirth || '') && (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-sm text-gray-900">{formatDate(registration.createdAt)}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center space-x-1">
                          ...
                          <span className={`text-xs ${registration.medicalCertificateFile ? 'text-green-600' : 'text-red-600'}`}>
                            {registration.medicalCertificateFile ? 'Certificat OK' : 'Certificat manquant'}
                          </span>
                        </div>
                      </div>
                    </td>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <span className="text-gray-900">{selectedRegistration.user?.firstName} {selectedRegistration.user?.lastName}</span>
                  </div>
                  <div>
                    <span className="font-medium text-gray-700">Âge : </span>
                    <span className="text-gray-900">{calculateAge(selectedRegistration.user?.dateOfBirth || '')} ans</span>
                  </div>
                  <div>
                    <span className="font-medium text-gray-700">Date de naissance : </span>
                    <span className="text-gray-900">{new Date(selectedRegistration.user?.dateOfBirth || '').toLocaleDateString('fr-FR')}</span>
                  </div>
                  <div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-gray-900">{selectedRegistration.user?.email}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Phone className="h-4 w-4 text-gray-400" />
                    <span className="text-gray-900">{selectedRegistration.user?.phone}</span>
                  </div>
                </div>
              </div>

              {/* Parent Info (if minor) */}
              {isMinor(selectedRegistration.user?.dateOfBirth || '') && (
                <div className="bg-blue-50 rounded-lg p-4">
                  <h3 className="font-medium text-gray-900 mb-3">Représentant légal</h3>
                  <div className="space-y-2">
                    <div>
                      <span className="font-medium text-gray-700">Nom : </span>
                      <span className="text-gray-900">À renseigner</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Mail className="h-4 w-4 text-gray-400" />
                      <span className="text-gray-900">À renseigner</span>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="space-y-2">
                  ...
                  <div>
                    ...
                    <span className={selectedRegistration.medicalCertificateFile ? 'text-green-600' : 'text-red-600'}>
                      {selectedRegistration.medicalCertificateFile ? 'Fourni' : 'À fournir'}
                    </span>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span className="text-gray-900">{formatDate(selectedRegistration.createdAt)}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex space-x-3">
                  {selectedRegistration.status === 'pending' && (
                    <>
                      <button
                        onClick={() => {
                          updateStatus(selectedRegistration.id, 'validated');
                          closeDetail();
                        }}
                        className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-medium transition-colors duration-200 flex items-center space-x-2"
                      >
                  ...
                  <a
                    href={`mailto:${selectedRegistration.user?.email}?subject=Inscription Judo Club Saint Pathus&body=Bonjour ${selectedRegistration.user?.firstName},%0D%0A%0D%0AMerci pour votre demande d'inscription.%0D%0A%0D%0ACordialement,%0D%0AJudo Club Saint Pathus`}
